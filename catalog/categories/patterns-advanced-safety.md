...
### Reference impl: pr-review-toolkit 5-tier rubric

The `pr-review-toolkit` plugin codifies a more granular 5-tier rubric with named semantics and a hard suppression rule at ≥80:

| Score | Tier | Action |
|-------|------|--------|
| 91-100 | **Certain** | Report as a confirmed issue; recommend specific fix. |
| 76-90 | **Likely** | Report with the supporting evidence; offer alternative interpretation. |
| 51-75 | **Suggestive** | Report only if security; otherwise convert to a question for the author. |
| 26-50 | **Speculative** | Suppress unless explicitly asked for low-confidence findings. |
| 0-25  | **Noise** | Suppress unconditionally. |

**The ≥80 hard suppression rule:** for non-security reviews, findings below 80 are converted to questions, not assertions ("Is this intentional?"). The rationale: reviewers below 80 confidence routinely produce drive-by noise that erodes trust in the tool. Forcing question-form keeps the agent humble where it should be.
...
**Rules:**
- Any `git`/`github` source MUST include a `sha` — `ref` alone is mutable.
- `http` sources MUST include a `sha256`.
- A skill upgrade is the act of replacing the SHA, not editing in-place.

**Why this works:** Mutable refs (`main`, `v1.4.2` tag) can be force-pushed. A SHA pin means the install is deterministic — every consumer gets identical bytes. The polymorphic source schema lets registries accept internal and public sources side-by-side without losing the integrity guarantee.
...
**Why this fails:** No SHA pin — the maintainer (or an attacker who compromises the repo) can change what `main` points to silently. No source-type polymorphism — internal git and HTTP archives can't be expressed. No integrity field for archives — a tampered tarball installs with no warning.

---

## Pattern 155: Plugin Lifecycle State Machine

**Prevalence:** Multi-source: `marketplace.json` entries across claude-plugins-official + community registries
**Related patterns:** [Version Check](/prompt-context-patterns/catalog/categories/patterns-quality-and-feedback#pattern-30-version-check--update-notification), [Marketplace Source-Type Polymorphism](#pattern-153-marketplace-source-type-polymorphism-sha-pinned)

**What it is:** Marketplace entries declare a lifecycle state (`active` / `deprecated` / `archived`) with required metadata: a sunset window, a `deprecatedMessage`, and (for `deprecated`) a `replacedBy` pointer. Distinct platform-governance concern: marketplaces need to retire plugins safely without breaking installed consumers.
...
**Rules:**
- `state: deprecated` REQUIRES `sunsetAt`, `deprecatedMessage`, and `replacedBy`.
- `state: archived` REQUIRES `sunsetAt` to be in the past; installs are blocked.
- On install of a `deprecated` plugin: show `deprecatedMessage` and pause for confirmation.
- Within 30 days of `sunsetAt`: show a stronger warning.

**Why this works:** The state machine separates "still works but don't start new installs" (deprecated) from "won't work, don't even try" (archived). Required `replacedBy` forces maintainers to provide a migration path — not just a tombstone. Sunset window gives consumers time to migrate.
...
**Why this fails:** A boolean `deprecated` flag with no sunset date means consumers can't tell when the plugin will actually stop working. No `replacedBy` means deprecation surfaces a problem without offering a solution. No state distinction between "deprecated" and "archived" means the registry can't enforce different install behaviors.
