...
**Source research:** Claude Code CLI's composable system prompt — 110+ extracted fragments.
...
### Variant: SessionStart-hook-as-persona-installer

A specific assembly mechanism worth naming: a `SessionStart` hook that emits `additionalContext` via a heredoc, installing a persona/output-style on every new session without modifying the system prompt:
...
The hook output is appended to the session prompt as context — composition without source-edit. Useful for per-repo personas (the hook lives in `./.claude/`) that travel with the working directory.
...
### Prompt-layer fallback (when platform tool-removal isn't available)

When you can't configure `disallowedTools` (e.g. the host doesn't expose tool-set restrictions for the subagent type you need), fall back to a verbatim-quoted prompt:
...
The verbatim-quote framing and the acknowledge-by-repetition step are the load-bearing details — they prevent the model from softening the rule into "try to avoid editing".
...
## Pattern 147: Announce-Skill-On-Entry Protocol

**Prevalence:** Multi-source (5 superpowers skills + brainstorming + using-superpowers + write-a-skill)
**Related patterns:** [Output Visibility Awareness (131)](/prompt-context-patterns/catalog/categories/patterns-claude-code-platform#pattern-131-output-visibility-awareness), [Skill Composition (19)](/prompt-context-patterns/catalog/categories/patterns-agent-orchestration#pattern-19-skill-composition--cross-skill-invocation)

**What it is:** Every skill begins with a mandatory one-line announcement of which discipline the agent is now operating under, before any work begins. Distinct from Pattern 131 (which is about whether *output* is seen) — this is about declaring *which rule-set is active* when multiple skills could apply.

**When to use:**
- Skill-composition environments where multiple skills can be active concurrently
- Workflows where the user benefits from knowing which discipline is being applied
- When the agent might invoke a sub-skill without making it observable
...
**Why this works:** The single-line format is cheap (no token bloat) but provides observability — the user sees which discipline is active and can call out drift. "Soft commitment device" exploits the model's coherence pressure: having announced "tdd", the model is more likely to follow tdd's rules than if the skill were silently loaded.
...
**Why this fails:** Silent skill loading means the user can't tell which discipline produced the output. When two skills conflict, there's no record of which one won. The agent can also silently drift to a different skill mid-task with no observable signal.
