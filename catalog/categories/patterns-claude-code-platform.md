...
**Source research:** The full system prompt powering Claude Code CLI.
...
### Worked example: file-organizer / invoice-organizer (sample-diff-then-approve)

The `file-organizer` and `invoice-organizer` skills apply this pattern to destructive filesystem ops by writing a *proposed* rename/move plan to a file and asking the user to approve the diff before any move is executed:

```markdown
## Phase 2: Plan (no writes yet)

1. Walk the target directory; classify each file by type.
2. Write the proposed plan to `./.organize-plan.tsv`:
     <current-path>\t<proposed-path>\t<reason>
3. Show the user a summary: N moves, M renames, K skipped.
4. Ask: "Approve, edit, or cancel?"

## Phase 3: Execute (only on approval)

Read the (possibly edited) plan file. Apply moves with `git mv` where the
target is in a git repo, `mv` otherwise. NEVER move outside the plan file.
```

This converts an irreversible bulk operation into an approve-the-diff gate. The plan file is itself the investigation artifact — the user can see exactly what the agent observed and intends to do.
...
### Variant A: Prompt-type hook (LLM decides)

Hooks don't have to be deterministic shell scripts. A "prompt-type" hook delegates the decision to a fresh LLM call:
...
```json
{
  "PreToolUse": [{
    "matcher": "Bash",
    "type": "prompt",
    "prompt": "The agent is about to run: {{tool_input.command}}. Is this safe in the current repo? Respond with PASS or BLOCK <reason>."
  }]
}
```

This lets the hook apply judgment (rather than a fixed pattern) — useful when the safety question depends on context the regex can't see. The hook LLM has no memory of the parent session, so injection through the hook prompt is bounded.

### Variant B: Path-triggered just-in-time security reminder

A hook that doesn't *block* — it injects a reminder when the agent touches a sensitive path:

```json
{
  "PreToolUse": [{
    "matcher": "Edit",
    "pathPattern": "**/secrets/**|**/.env*|**/credentials*",
    "type": "reminder",
    "message": "You are editing a secrets/credentials path. Verify the change does not commit plaintext keys, and prefer reading from env vars in code."
  }]
}
```

Pairs with Pattern 136 (Security Monitor Agent) — the reminder is a lightweight inline check; the monitor is the dedicated agent. Both can coexist.
