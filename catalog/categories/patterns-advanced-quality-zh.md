...
### 顺序变体：Spec-then-Quality（subagent-driven-development）

当并行审查不可行时（例如一个实现者加上串行的多个审查者），同样的模式可以顺序派发：

```
implementer → spec-reviewer (iterate until pass) → quality-reviewer (iterate until pass) → done
```

顺序很重要：spec-reviewer 先验证实现与 spec 是否一致（功能正确性）；只有通过之后，quality-reviewer 才评估风格、测试和可维护性。颠倒顺序会让 quality-review 的精力浪费在因 spec 原因可能要重写的代码上。

### 验证器设计：剥离推理 + 命名失败枚举

源自数学奥林匹克的用例：当在 solver 的*输出*上运行一个验证器时，**在把输出传给验证器之前先剥离 solver 的推理过程**。验证器应该只从答案本身独立推导正确性 —— 而不是被 solver 的叙述所暗示。把这一点与一份枚举出的失败模式清单配套使用：
...
剥离推理可以阻止验证器继承 solver 的确认偏差；命名的失败模式可以避免出现笼统的"看起来还行"的判定。
...
## Pattern 149: Pressure-Scenario TDD for Skill Authoring (用压力场景 TDD 来编写 Skill)

**出现频率：** superpowers/writing-skills 中的独立技术；高度元层次新颖性
**相关模式：** [Eval-Driven Skill Improvement Loop](/prompt-context-patterns/catalog/categories/patterns-open-source-skills-zh#pattern-119-eval-driven-skill-improvement-loop), [Rationalization-Prevention Table](/prompt-context-patterns/catalog/categories/patterns-quality-and-feedback-zh#pattern-146-rationalization-prevention-table)

**定义：** 在编写一个 skill 时，你写的测试用例要模拟*一个基线 agent 在压力下会产生的合理化借口* —— 并断言 skill 文本能让 agent 抵抗这些借口。与 Pattern 119 不同（eval 测试的是 skill 的*输出*）；这里测试的是 skill 的*内容本身*能否承受 agent 面对的社交/时间压力而不跳步。
...
**为什么有效：** 一个只在配合性条件下能通过的 skill 没有价值 —— agent 本来就会做对。压力场景才是 skill 真正赚到饭钱的地方。"无 skill 基线"与"加载 skill 后"的对照衡量的是 skill 的实际增益，而不是它表面看起来是否有理。
...
**为什么失败：** "看起来不错"衡量的是表面合理性，而不是 agent 在压力下的抵抗力。skill 作者本身已经知道规则，又用配合性的测试用例去确认 —— 而那正是这条规则原本就不必要的场合。skill 发布时显得很强，但用户第一次推回时就败下阵来。
