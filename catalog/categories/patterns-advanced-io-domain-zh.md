...
**为什么有效：** 每一条原则都针对只在调用方是 LLM 时才出现的失败模式。原则 2（上下文预算）是从人类面向的 API 移植过来的工具最常违反的一条。原则 3 把错误转化为"下一步动作"的提示，利用了 agent 倾向于遵循工具输出中具体指令的特性。
...
**为什么失败：** "为每个 endpoint 包一个工具"会产出 40 个工具，而 6 个用户意图就够了 —— agent 仅在区分同胞工具上就要烧掉上下文。"返回完整 JSON 响应"会无声地吃掉上下文窗口。"标准 HTTP 错误码"让 agent 猜该对 400 做什么；它常常会原样重试。
