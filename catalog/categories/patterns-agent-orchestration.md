...
**Related foundational techniques:** Cognitive Offloading, Token-Action Binding (see [prompt-engineering-for-skills.md](/prompt-context-patterns/catalog/techniques/token-level-techniques))
...
**Why this works:** The three-column table (Task / Tool / NOT these) makes the routing unambiguous. The "NOT these" column explicitly blocks the model's tendency to use general-purpose tools (qa-bot, enterprise_chat) that return unreliable results. The rationale is stated upfront ("unreliable, unstructured results"). Tool names use exact MCP identifiers, not descriptions. Even non-obvious tools are included (screenshots → `Read`, crash dumps → `Bash` with gdb).
...
**Why this fails:** "Appropriate" and "best" require the model to evaluate tool quality — it will default to familiar general-purpose tools (qa-bot) even when precision tools exist. No prohibited tools means the model freely uses unreliable Q&A endpoints for structured queries. No mapping means the model might use a work item search tool to search code, or a code search tool to find PRs.

### Profile-stacked loadout (install-time complement)

Pattern 21 above is *runtime* routing — task → tool. A complementary mechanism is *install-time* routing: role → toolset, declared in a profile file:

```toml
# agency.toml — internal experiments
[role.reviewer]
...
Profiles are additive — when an agent is spawned with `role: reviewer`, the loadout is composed from base + role. This pre-decides the routing at the *capability* level so the runtime table only has to disambiguate within an already-narrow set. The two layers compose: the profile says "you have these tools at all", the runtime table says "use exactly this one for this task".
