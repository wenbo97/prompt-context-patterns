...
### Creation-avoidance sub-pattern (worktree / submodule detection)

A specialized form: before *creating* a new isolated workspace, probe for one that already exists. The `using-git-worktrees` skill applies this to avoid spawning duplicate worktrees:
...
# If we're already in a worktree, reuse it
if [ "$GIT_DIR" != "$GIT_COMMON" ]; then
  echo "already in worktree: $(pwd)"
  exit 0
fi

# False-positive guard: submodules also have GIT_DIR != GIT_COMMON
if [ -f "$GIT_DIR/../.gitmodules" ] && grep -q "$(basename $PWD)" "$GIT_DIR/../.gitmodules"; then
  echo "in submodule, not worktree — fall through to creation"
else
...
The submodule false-positive guard is the load-bearing detail — without it, the probe misclassifies submodule checkouts as worktrees and skips creating the actual workspace.
...
### Refinement: anti-convergence directive

A distinct discipline beyond named-failures: forbid *convergence across generations*. Even if Space Grotesk isn't slop in any single piece, picking it for the 5th project this week IS slop in aggregate.

```markdown
## Anti-Convergence Rule

Across multiple generations for different briefs:
- Do NOT default to the same typeface (e.g. Space Grotesk every time).
- Do NOT default to the same color move (always-warm-accent, always-monochrome).
- Do NOT default to the same layout primitive (always-asymmetric-2-column).

Each brief gets a deliberately different choice along at least 2 of {typeface, palette, layout}.
If you find yourself reaching for the same move, name it explicitly as a
default-to-avoid for this brief.
```

This rule operates at the *portfolio* level rather than the *piece* level — it's the difference between "no slop in this design" and "no slop in this designer's output overall".
...
### Refinements (added 2026-05)

- **Anti-staleness rule:** Always run `RUBE_SEARCH_TOOLS` first — never hardcode an action slug. Slugs change between API versions; a hardcoded `slack.chat.postMessage` breaks silently when the toolkit revises to `slack.send_message`.
- **ACTIVE-status check:** After `RUBE_MANAGE_CONNECTIONS`, verify the returned status is exactly `ACTIVE`. `PENDING` or `EXPIRED` connections execute and silently fail (or worse, succeed with stale credentials).
- **Session reuse:** Discovered tool schemas are stable for the session — cache the search result, don't re-search on every call.
- **Anti-example:**
  ```python
  # Hardcoded slug — breaks on API revision
  rube.execute("slack.chat.postMessage", {...})
  ```
  vs
  ```python
  tools = rube.search_tools("slack")
  send = next(t for t in tools if "send" in t.name)
  rube.execute(send.name, {...})
  ```
- **Prevalence evidence:** This 3-step template occurs 800+ times across the Composio skill corpus, making it the highest-prevalence pattern in the open-source ecosystem.
...
<a id="pattern-143-pre-set-catalog-with-generate-on-the-fly-fallback"></a>
## Pattern 143: Pre-Set Catalog with Generate-On-The-Fly Fallback

**Category:** Knowledge / Creative Workflow
**Source:** `awesome-claude-skills/theme-factory`, `brand-guidelines-generator`, `canvas-design` (3 skills)
**Prevalence:** 3 skills

### Concept

Offer the model a curated catalog of pre-baked options (themes, palettes, layouts) it can pick from — AND a fallback branch that generates a fresh option from scratch when none of the curated entries fit. Distinct from few-shot examples (Pattern 25): the catalog is the *primary* output path, generation is the *escape hatch*.

### Good: Catalog-first with named fallback branch

```markdown
## Step 1: Select a Theme

Pick the closest match from `./themes/` (15 curated themes):
- minimalist-mono, brutalist-grid, pastel-editorial, terminal-green,
  noir-magazine, sunset-gradient, scandinavian-clean, ...

If none fit the brief within ~70% similarity:
  → Branch to "Generate-On-The-Fly" (Step 1b)

## Step 1b: Generate-On-The-Fly (only if Step 1 missed)

Compose a new theme by combining:
- One palette from `./palettes/` (or invent one obeying the contrast rules)
- One typographic pair from `./type-pairs/`
- One layout grid from `./layouts/`

Document the generated theme as `theme-{name}.md` so it joins the catalog.
```

### Bad: Pure generation OR pure catalog

```markdown
## Theme Selection
Generate a fresh theme for the brief.
```

(No curated anchor — output drifts to the model's aesthetic prior, producing AI slop.)

```markdown
## Theme Selection
Pick one of: minimalist-mono, brutalist-grid, pastel-editorial.
```

(No escape hatch — when the brief is "art deco", the model must force-fit one of three wrong themes.)

### Why It Works

The catalog provides Grounding (Technique 2) and Few-Shot anchoring; the fallback prevents the catalog from becoming a hard constraint when the input space exceeds the curated space. The "document generated themes back into the catalog" loop makes the catalog grow over time — a self-improving knowledge base.
