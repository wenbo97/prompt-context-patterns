...
### 创建避免子模式（worktree / submodule 检测）

一个专门形式：在*创建*新的隔离工作区之前，先探测是否已存在一个。`using-git-worktrees` skill 用此避免重复创建 worktree:
...
# 误报防护：submodule 同样 GIT_DIR != GIT_COMMON
...
submodule 误报防护是关键细节 —— 没有它，探测会把 submodule checkout 误判为 worktree，从而跳过实际工作区的创建。
...
### 精炼: anti-convergence 指令

超出"命名失败"之外的另一层纪律：禁止*跨代际收敛*。即使 Space Grotesk 在任何单件作品里都不算水货，本周第 5 个项目都选它，整体上就是水货。

```markdown
## Anti-Convergence Rule

跨多次为不同 brief 的生成：
- 不要默认同一字体（如每次都 Space Grotesk）。
- 不要默认同一配色手法（总是暖色点缀、总是单色）。
- 不要默认同一布局原语（总是非对称双栏）。

每个 brief 在 {字体, 调色板, 布局} 中至少两个维度做出刻意不同的选择。
如果发现自己又要伸手去拿同一个手法，把它显式列为本 brief 要避免的默认项。
```

此规则作用于*组合*层级而非*单件*层级 —— 这是"本设计无水货"与"该设计师整体输出无水货"的差别。
...
- **反陈旧规则:** 始终先跑 `RUBE_SEARCH_TOOLS` —— 不要硬编码 action slug。API 版本变化时 slug 会变；硬编码的 `slack.chat.postMessage` 在 toolkit 改成 `slack.send_message` 时会静默失效。
- **ACTIVE 状态检查:** `RUBE_MANAGE_CONNECTIONS` 后，确认返回状态严格等于 `ACTIVE`。`PENDING` 或 `EXPIRED` 的连接会执行并静默失败（更糟：用过期凭据成功执行）。
- **会话复用:** 发现的工具 schema 在会话内是稳定的 —— 缓存 search 结果，不要每次调用都重搜。
- **反例:**
  ```python
  # 硬编码 slug —— API 改版即失效
  ...
  ```
- **普遍性证据:** 这个 3 步模板在 Composio skill 语料中出现 800+ 次，是开源生态中出现率最高的模式。
...
## Pattern 143: Pre-Set Catalog with Generate-On-The-Fly Fallback（预设目录 + 即时生成兜底）

**分类:** Knowledge / Creative Workflow
**来源:** `awesome-claude-skills/theme-factory`、`brand-guidelines-generator`、`canvas-design`（3 个 skill）
**出现率:** 3 个 skill

### 概念

给模型一份精选的预制选项目录（主题、配色、布局）让它挑选 —— 同时给一个兜底分支，当目录中没有合适项时即时生成全新选项。区别于 few-shot 示例（Pattern 25）：目录是*主*输出路径，生成是*逃生*通道。
...
从 `./themes/` 中挑最接近的（15 个精选主题）：
- minimalist-mono, brutalist-grid, pastel-editorial, terminal-green,
  noir-magazine, sunset-gradient, scandinavian-clean, ...

若没有任何主题与 brief 相似度 ~70% 以上：
  → 跳转 "Generate-On-The-Fly"（Step 1b）

## Step 1b: 即时生成（仅当 Step 1 未命中）

由以下组件组合一个新主题：
- 从 `./palettes/` 取一个配色（或在对比度规则下自创一个）
- 从 `./type-pairs/` 取一对字体
- 从 `./layouts/` 取一种布局栅格

将生成的主题写为 `theme-{name}.md`，使其加入目录。
```
...
（无精选锚点 —— 输出漂向模型的审美先验，产出 AI 水货。）

```markdown
## 主题选择
从以下三个中挑一个：minimalist-mono, brutalist-grid, pastel-editorial。
```

（无逃生通道 —— 当 brief 是 "art deco" 时，模型被迫硬塞错误主题。）

### 为什么有效

目录提供 Grounding（Technique 2）与 Few-Shot 锚定；兜底分支防止目录变成硬约束（当输入空间超出精选空间时）。"将生成的主题回写入目录"的循环让目录随时间增长 —— 一个自我改进的知识库。
