...
### Subagent context construction (superpowers corroboration)

Three superpowers skills (`subagent-driven-development`, `dispatching-parallel-agents`, `requesting-code-review`) corroborate the stronger form of this rule:

> **Subagents never inherit session history.** The orchestrator constructs precisely the context the subagent needs — no more, no less.

Practical implications:
- Pass file paths, not file contents.
- Pass a one-paragraph problem statement crafted for the subagent, not the user's original message.
- Pass acceptance criteria as a checklist, not as conversational prose.
- Never assume the subagent has seen prior agent output — restate dependencies.

This converts the orchestrator's job from "delegate work" to "author a minimal brief" — and the brief itself becomes the artifact you can review for quality.
