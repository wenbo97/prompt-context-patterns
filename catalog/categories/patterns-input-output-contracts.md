...
### Plugin-portable paths: `${CLAUDE_PLUGIN_ROOT}`

When a skill is packaged as a plugin loaded into someone else's environment, *no absolute path is safe*. The plugin doesn't know its own install location at write time. Use the platform's plugin-root indirection:
...
**Banned in plugin manifests:**
- Absolute paths: `/Users/alice/...`, `C:\Users\alice\...`
- CWD-relative paths: `./scripts/check.sh` (CWD at hook invocation is unpredictable)
- Home-relative paths: `~/scripts/check.sh` (tilde is shell-expanded, not JSON-expanded)

The `${CLAUDE_PLUGIN_ROOT}` env var is set by the host at load time to the plugin's install directory — the only path guaranteed to resolve correctly on every consumer's machine.
