...
**相关基础技术：** Schema Priming, Grounding/Anchoring, Cognitive Offloading（见 [prompt-engineering-for-skills.md](/prompt-context-patterns/catalog/techniques/token-level-techniques-zh)）
...
### Plugin 可迁移路径：`${CLAUDE_PLUGIN_ROOT}`

当 skill 作为 plugin 打包并加载到他人环境时，*没有任何绝对路径是安全的*。Plugin 在写入时并不知道自己的安装位置。使用平台的 plugin-root 间接引用：
...
**Plugin 清单中被禁的写法：**
- 绝对路径：`/Users/alice/...`、`C:\Users\alice\...`
- CWD 相对路径：`./scripts/check.sh`（hook 调用时的 CWD 不可预测）
- 家目录相对路径：`~/scripts/check.sh`（波浪线是 shell 展开，不是 JSON 展开）

`${CLAUDE_PLUGIN_ROOT}` 环境变量由宿主在加载时设置为 plugin 的安装目录 — 这是唯一能在每个消费者机器上正确解析的路径。
