...
**相关基础技术：** Schema Priming, Negative Space, Cognitive Offloading（见 [prompt-engineering-for-skills.md](/prompt-context-patterns/catalog/techniques/token-level-techniques-zh)）
...
**何时使用:**
- 与任何 Iron-Law 框架下、模型曾反复跳过的规则配对使用
- 验证关、test-first 协议、修复前先复现的工作流
- 任何模型典型失败模式是"说服自己这是特殊情况"的场景
...
如果发现自己在按以下任何思路推理，停。你正要跳过一步。重读 Iron Law。

| 合理化 | 现实 |
|--------|------|
| "改动很小 —— 不用测试。" | 小改动恰恰是回归藏身之处。测试花 2 分钟；回归花 2 天。 |
| "我已经知道 bug 是什么 —— 直接修。" | 跳过复现就无法验证修复。你会发出去另一个 bug。 |
| "跑全套测试太慢 —— 我信单元测试。" | 你跳过的那个集成测试，恰好就是会抓住回归的那一个。 |
| "用户很急 —— 我合完再验证。" | "合完再验"永远不会发生。Bug 上线。 |
| "和上次修复类似 —— 一样的办法应该行。" | 上次成功是因为你验证了。这次跳过验证就是"类似"和"相同"的差别。 |

如果出现一个不在表中的新合理化，把它加到表里，留给下一个 Agent。
```

**为什么有效:** 预先列举借口让模型在自己的推理模式*形成时*就认出 —— 而不是在损失造成之后。"现实"列不只说"不"，它说明该合理化所开启的具体失败模式。"加入新借口"的循环把表变成会随每次违反而进化的活文物。
...
**为什么失败:** "不要跳过"与规则本身重复。"自信不是证据"是一条抽象原则 —— 模型可以*一边同意一边继续跳过*，因为此刻这个 case 感觉不同。不命名具体合理化，模型就认不出自己正在产出它们。
