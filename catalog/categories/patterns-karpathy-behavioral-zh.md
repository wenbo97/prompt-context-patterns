...
| K6: Goal-Reformulation to Verifiable Test | 模糊目标产出模糊结果 | "Restate the goal as a test that would catch the failure" |

---

## Pattern K6: Goal-Reformulation to Verifiable Test（目标重写为可验证测试）

**类别:** Execution Control（K4: Goal-Driven Execution 的延伸）

**它解决的问题:** 用户经常把目标用名词形式抛给 Agent（"更好的校验"、"更健壮的错误处理"）。Agent 自己填补解读、产出看起来合理的东西，用户无法判断目标是否达成。分歧到 review 时才浮现，而不是工作开始前。

**技术:** 开工前，把目标重写成*可验证测试* —— 一个具体的输入/期望输出对（或属性断言），今天会失败、做完后会通过。如果你写不出这个测试，说明你还没理解目标。

### 重写表

| 用户陈述的目标 | 可验证测试 |
|---------------|-----------|
| "加校验"                       | "提交 `{email: ''}` 返回 400，且 `errors.email = 'required'`" |
| "更健壮的错误处理"             | "请求中途杀掉 DB，返回 503（不是 500）且日志含 `db_disconnect`" |
| "模块 X 的更好测试"            | "对 X 做变异测试，killed mutants >=80%（当前 42%）" |
| "handlers/ 代码更干净"         | "ESLint complexity 规则阈值 10 通过（当前 18）" |
| "更快的启动"                   | "`time node app.js --probe-ready` 冷启动 <800ms（当前 2.4s）" |

### 步骤

1. 阅读用户的目标。
2. 用一句话起草可验证测试（输入 -> 可观测结果）。
3. 如果有歧义空间，问用户"你是这个意思吗？"
4. 在实现之前写出失败的测试（或跑出失败的测量）。
5. 当且仅当测试通过时，实现才算完成。

**为什么有效:** 把目标强行过一遍"可验证测试"过滤器，会在*实现之前*暴露歧义。如果用户想要"邮箱格式校验"但 Agent 的测试是"非空校验"，第 3 步就会暴露分歧 —— 零成本。第 4 步的失败测试也兼任 Karpathy K4（Goal-Driven Execution）的验证步骤。

**在我们插件中的应用:** 任何用户陈述的改进目标、重构请求或"让 X 更好"的指令。当目标可以在多个质量层次上被满足时尤其有价值 —— 测试钉住具体层次。

**与现有模式的关系:** K4 的更锐利版本。K4 说"每个步骤有 -> verify:"，K6 说"整个目标有 -> verify:"。
