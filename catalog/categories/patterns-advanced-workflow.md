...
**Why this works:** Models trained on conversational data have a strong prior toward "checking in" between tasks — which kills throughput in autonomous execution. Naming the prohibition removes the ambiguity. The three explicit exceptions prevent the rule from becoming a license for reckless execution.
...
**Why this fails:** "As needed" defaults to "after every task" because that's the conversational prior. A 12-task plan becomes 12 round-trips with the user, defeating the entire purpose of autonomous execution.

---

## Pattern 154: Self-Looping Stop Hook (Ralph Loop)

**Prevalence:** Distinct orchestration model in `ralph-loop` plugin; generalizable
**Related patterns:** [Hook-Driven Automation](/prompt-context-patterns/catalog/categories/patterns-claude-code-platform#pattern-132-hook-driven-automation-awareness), [Loop Prevention with Max Iterations](/prompt-context-patterns/catalog/categories/patterns-advanced-orchestration#pattern-41-loop-prevention-with-max-iterations)

**What it is:** A Stop-event hook that re-enters the same session with a "continue working" prompt, creating an autonomous loop without manual re-invocation. The prompt-side discipline says the agent must NOT produce false "I'm done" signals just to escape the loop.
...
**Why this works:** The Stop hook closes the loop without operator intervention. The prompt-side honesty constraint addresses the model's failure mode of producing convenient lies to terminate. Separating DONE and BLOCKED prevents the model from blurring "I finished" with "I gave up".
...
(With no prompt-side discipline.)

**Why this fails:** Without the honesty constraint, the model will emit "I'm done" the first time it hits a hard problem — the loop terminates with the work incomplete. Without DONE/BLOCKED differentiation, the user can't tell completion from giving up.
