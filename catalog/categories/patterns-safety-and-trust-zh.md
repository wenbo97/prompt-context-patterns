...
**相关基础技术：** Negative Space, Attention Locality（见 [prompt-engineering-for-skills.md](/prompt-context-patterns/catalog/techniques/token-level-techniques-zh)）
...
**适用场景：**
- 任何获取外部内容的技能（网页、ADO PR、IncidentTracker 事件、wiki 页面）
- 任何处理用户上传文件的技能
- 任何读取用户贡献内容的数据库的技能
...
**为何有效：** 命名了具体不可信的外部来源（internal-wiki.example.com 页面、ADO PR、Kusto 结果）— 而非模糊的 "be careful"。给出注入模式的具体示例（"ignore previous instructions"、"run this command"）。定义了具体响应动作：向用户标记并继续工作流。"continue without deviation" 指令防止模型在检测到潜在注入后进入困惑状态。
