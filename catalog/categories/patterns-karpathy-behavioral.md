...
Build produces NU1701 for Contoso.LegacyClient. Agent thinks: "NU1701 is a warning, 
...
<ItemGroup Condition="'$(TargetFramework)' != 'net472'">
  <PackageReference Include="Contoso.LegacyClient" NoWarn="NU1701" PrivateAssets="all" />
</ItemGroup>
```
```
...
| K6: Goal-Reformulation to Verifiable Test | Vague goals producing vague outputs | "Restate the goal as a test that would catch the failure" |

---

## Pattern K6: Goal-Reformulation to Verifiable Test

**Category:** Execution Control (extends K4: Goal-Driven Execution)

**Problem it solves:** Users often hand the agent a goal stated as a noun ("better validation", "more robust error handling"). The agent fills in its own interpretation, produces something plausible, and the user has no way to tell whether the goal was met. The disagreement surfaces during review, not before work began.

**Technique:** Before starting, restate the goal as a *verifiable test* — a concrete input/expected-output pair (or a property assertion) that would fail today and pass after the work is done. If you can't write the test, you don't understand the goal yet.

### Reformulation Table

| User-stated goal | Verifiable test |
|------------------|-----------------|
| "Add validation"                | "Submitting `{email: ''}` returns 400 with `errors.email = 'required'`" |
| "More robust error handling"    | "Killing the DB mid-request returns 503 (not 500) AND logs `db_disconnect`" |
| "Better tests for module X"     | "Mutation testing on X shows >=80% killed mutants (current: 42%)" |
| "Cleaner code in handlers/"     | "ESLint complexity rule passes with threshold 10 (current: 18)" |
| "Faster startup"                | "`time node app.js --probe-ready` reports <800ms cold (current: 2.4s)" |

### Procedure

1. Read the user's goal.
2. Draft the verifiable test in one sentence (input → observable outcome).
3. Ask the user "is this what you mean?" if there's room for disagreement.
4. Write the failing test (or run the failing measurement) BEFORE implementation.
5. Implementation is complete iff the test passes.

**Why this works:** Forcing the goal through the "verifiable test" filter exposes ambiguity *before* implementation. If the user wanted "validation on email format" but the agent's test is "validation on emptiness", the disagreement surfaces in step 3 — at zero cost. The failing test in step 4 also doubles as Karpathy's K4 (Goal-Driven Execution) verification step.

**When to use in our plugin:** Any user-stated improvement goal, refactor request, or "make X better" instruction. Especially valuable when the goal could be satisfied at many quality levels — the test pins the level.

**Relationship to existing patterns:** Sharper version of K4. Where K4 says "each step has → verify:", K6 says "the whole goal has → verify:".
