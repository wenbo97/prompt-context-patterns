...
### Subagent 上下文构造（superpowers 的佐证）

superpowers 中的三个 skill（`subagent-driven-development`、`dispatching-parallel-agents`、`requesting-code-review`）共同佐证了这一规则的更强形式：

> **Subagent 从不继承会话历史。** 编排器为 subagent 精确构造其所需的上下文 —— 不多不少。

实际含义：
- 传文件路径，不传文件内容。
- 传一段为 subagent 量身定制的问题陈述，而不是用户的原始消息。
- 用清单形式传验收标准，而不是对话式散文。
- 永远不要假设 subagent 看过前面 agent 的输出 —— 重述依赖项。

这把编排器的工作从"派活"变成"撰写最小化简报" —— 简报本身就成为你可以审查质量的产物。
