...
**Related foundational techniques:** Grounding/Anchoring, Few-Shot with Embedded Reasoning, Attention Locality (see [prompt-engineering-for-skills.md](/prompt-context-patterns/catalog/techniques/token-level-techniques))
...
- **Table 2 — ServiceCatalog to on-call team**: Match ServiceCatalog service or application names
  to their owning incident team.
- **Table 3 — Process to on-call team**: Match crashing process names to their owning incident team.
...
3. If the user names an **ServiceCatalog service** → look up in Table 2
...
3. **Every gdb command run** on every `.dmp` / `.mdmp` file — with the output file path
