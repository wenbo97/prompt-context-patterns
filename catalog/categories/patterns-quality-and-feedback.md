...
**Link:** [Contoso Tools Pulse](https://https://forms.example.com/ContosoToolsPulse) (anonymous, 2 min)
...
**When to use:**
- Pair with any Iron-Law-framed rule that the model has historically skipped
- Verification gates, test-first protocols, repro-before-fix workflows
- Any place where the model's failure mode is convincing itself it's a special case
...
If you find yourself reasoning along any of these lines, STOP. You are about
to skip a step. Re-read the Iron Law.

| Rationalization | Reality |
|-----------------|---------|
| "The change is trivial — no test needed." | Trivial changes are where regressions hide. The test takes 2 min; the regression takes 2 days. |
| "I already know what the bug is — I'll just fix it." | If you skip the repro, you can't verify the fix. You'll ship a different bug. |
| "Running the full test suite is slow — I'll trust the unit tests." | The integration test you skipped is exactly the one that would have caught the regression. |
| "The user is in a hurry — I'll verify after merging." | "After merging" never happens. The bug ships. |
| "This is similar to the last fix — same approach should work." | The last fix succeeded because you verified it. Skipping verification this time is the difference between similar and identical. |

If a new rationalization appears that isn't in this table, add it here for
the next agent.
```

**Why this works:** Pre-enumerating excuses means the model recognizes its own reasoning pattern *as it forms* — not after the damage is done. The "Reality" column doesn't just say "no", it explains the specific failure mode the rationalization enables. The "add new excuses" loop turns the table into a living artifact that improves with each violation.

### Negative Example

```markdown
Don't skip the verification step, even if you feel confident.
Confidence is not a substitute for evidence.
```

**Why this fails:** "Don't skip" is the same instruction the rule already gave. "Confidence is not evidence" is an abstract principle the model can agree with *while still skipping the step* — because in the moment, this case feels different. Without naming the specific rationalizations, the model doesn't recognize itself producing them.
