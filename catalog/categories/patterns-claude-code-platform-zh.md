...
**数据来源:** Claude Code CLI 的完整 system prompt。
...
### 工作示例：file-organizer / invoice-organizer（采样-diff-然后-审批）

`file-organizer` 与 `invoice-organizer` skill 将此模式应用于破坏性文件系统操作：先将 *拟定* 的 rename/move 计划写入文件，并请用户审批 diff，然后才执行任何移动：
...
这把不可逆的批量操作转换为审批-diff 闸门。计划文件本身就是调查产物 — 用户可以准确看到 agent 观察到什么并打算做什么。
...
### 变体 A：Prompt 型 hook（让 LLM 决定）

Hook 不必是确定性的 shell 脚本。"Prompt 型"hook 把决定委托给一次新的 LLM 调用：
...
这让 hook 应用判断（而非固定模式）— 当安全问题取决于 regex 看不到的上下文时尤为有用。Hook LLM 不持有父会话的记忆，因此通过 hook prompt 的注入是有边界的。

### 变体 B：路径触发的 just-in-time 安全提醒

一个不 *阻止* 的 hook — 它在 agent 触碰敏感路径时注入一条提醒：
...
与 Pattern 136（Security Monitor Agent）配对 — 提醒是轻量内联检查；监控器是专用 agent。两者可共存。
