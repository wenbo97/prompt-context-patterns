...
3. General app telemetry → analytics-reference
...
```markdown
## Tool Design Principles (when the caller is an agent)

1. **Workflow, not endpoints** — Expose one tool per *user intent*, not one tool
   per REST endpoint. `create_or_update_issue(title, body)` not
   `POST_issues + PATCH_issues_id`.

2. **Context budget is scarce** — A tool that returns 50KB of JSON poisons the
   next 5 turns. Default to summaries; offer a `verbose=true` opt-in or a
   pagination token. Never return raw HTML or full file contents when a count,
   path list, or first-N-lines would do.

3. **Actionable error messages** — Errors must name the next call.
   - Bad: `400 Bad Request`
   - Good: `Missing required field 'project_id'. Call list_projects() to get IDs.`

4. **Schemas double as documentation** — Every parameter description must say
   what valid values look like AND why the agent would choose this tool over
   a sibling.

5. **Idempotency by default** — Agents retry on ambiguous failure. Tools must
   be safe to call twice with the same input.
```

**Why this works:** Each principle attacks a failure mode that only surfaces when the caller is an LLM. Principle 2 (context budget) is the one most often violated by tools ported from human-facing APIs. Principle 3 turns errors into prompts for the next action, exploiting the agent's tendency to follow concrete instructions in tool output.

### Negative Example

```markdown
## Tool Design
- Wrap each API endpoint as a tool.
- Return the full JSON response so the agent has all the info.
- Use standard HTTP error codes for failures.
```

**Why this fails:** "Wrap each endpoint" produces 40 tools where 6 user intents would do — the agent burns context just disambiguating sibling tools. "Full JSON response" silently consumes the context window. "Standard HTTP error codes" leaves the agent guessing what to do about a 400; it often retries the same call.
