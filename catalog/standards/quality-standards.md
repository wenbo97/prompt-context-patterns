...
|---------|-------------------|---------------------|
| Imperative mood | "You might want to check the csproj" | "Read the target csproj. If `<TargetFrameworks>` is absent, stop — this is a legacy LegacyBuild project." |
...
<examples>
<example>
<scenario>Contoso.LegacyClient triggers NU1701 — no REPLACE mapping exists</scenario>
<before>
<ItemGroup>
  <PackageReference Include="Contoso.LegacyClient" />
</ItemGroup>
</before>
<after>
...
<ItemGroup Condition="'$(TargetFramework)' != 'net472'">
  <!-- NU1701: Contoso.LegacyClient targets net472 only; suppress fallback warning -->
...
| **Tool name without parameters** ("use search_code") | Agent guesses parameters; uses wrong project name or missing filters | Agent calls `search_code` with `project: "contoso-control-plane"` instead of `project: "Contoso Core"` | Show the exact tool invocation: `search_code(searchText: "Contoso.WorkflowKit AND TargetFrameworks ext:csproj", project: ["Contoso Core"], repository: ["contoso-control-plane"], top: 15)` |
