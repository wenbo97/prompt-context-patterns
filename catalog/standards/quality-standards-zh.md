...
|------|-------------------|-----------------|
| 祈使语气 | "你可能想检查一下 csproj" | "读取目标 csproj。如果 `<TargetFrameworks>` 不存在，停止 — 这是一个旧式 LegacyBuild 项目。" |
...
| **工具名但没参数**（"use search_code"） | Agent 猜测参数；使用错误的项目名或缺少过滤器 | Agent 用 `project: "contoso-control-plane"` 而非 `project: "Contoso Core"` 调用 `search_code` | 展示确切的工具调用：`search_code(searchText: "Contoso.WorkflowKit AND TargetFrameworks ext:csproj", project: ["Contoso Core"], repository: ["contoso-control-plane"], top: 15)` |
